...
export const GET_admin_user$id: JetFunc<{ params: { id: string } }> = async (
  ctx,
) => {
  ...
  const userinfo = await user.query.select("*").where("id = ?", ctx.params.id)
    .getOne();
  if (!userinfo) {
    ctx.send({
      message: "User not found",
  ...
  const files = await file.query.select("*").where("userId = ?", ctx.params.id)
    .get();
  const vouchers = await voucher.query.select("*").where(
    "createdBy = ?",
    ctx.params.id,
  ).get();
  data.user = userinfo;
...
export const GET_admin_users: JetFunc<
  { query: { next: number; limit: number } }
> = async (ctx) => {
...
export const GET_admin_ban_user$id: JetFunc<{ params: { id: string } }> =
  async (ctx) => {
    if (!ctx.state.isAdmin) {
      ctx.send({
        message: "Unauthorized",
